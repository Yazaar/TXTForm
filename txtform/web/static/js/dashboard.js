(function () {
  ...
  document
    .querySelector('#myAccountBTN')
    .addEventListener('click', function () {
      myAccountUsername.value =
        myAccountUsername.getAttribute('data-username') || '';
      myAccountError.innerHTML = '';
      myAccountOverlay.classList.add('show');
    });

  document
    .querySelector('#saveMyAccountBTN')
    .addEventListener('click', async function () {
      var username = myAccountUsername.value;
      if (
        this.classList.contains('disabled') ||
        myAccountUsername.getAttribute('data-username') === username
      )
        return;
   ...
   * CONNECTIONS *
   ***************/
  var connectNewAccountOverlay = document.querySelector(
    '#connectNewAccountOverlay'
  );
  ...
  var disconnectAccountOverlay = document.querySelector(
    '#disconnectAccountOverlay'
  );
  var copyAccountSecret = document.querySelector('#copyAccountSecret');

  document
    .querySelector('#connectNewAccount')
    .addEventListener('click', function () {
  ...
  function editAccount(e) {
    ...
    disconnectAccountBTN.href =
      '/apis/' + targetType + '/disconnect/' + targetID;
    copyAccountSecret.setAttribute('data-accountType', targetType);
    copyAccountSecret.setAttribute('data-accountId', targetID);
    disconnectAccountOverlay.classList.add('show');
  }

  copyAccountSecret.addEventListener('click', async function () {
    ...
    try {
      var resp = await fetch(
        '/api/account/' + accountType + '/' + accountID + '/accountSecret'
      );
      var data = await resp.json();
    } catch (e) {
      var data = {};
    ...
    if (data.id_token) {
      navigator.clipboard.writeText(data.id_token);
    }
    this.innerText = lastText;
    setTimeout(() => {
      thisE.classList.remove('disabled');
    }, 3000);
  });
  ...
  var responseComponentOverlayE = document.querySelector(
    '#responseComponentOverlay'
  );
  var responseComponentSelect = document.querySelector(
    '#responseComponentSelect'
  );
  ...
  var deleteResponseIDInput =
    deleteResponseForm.querySelector('#deleteResponseID');
  ...
  document
    .querySelector('#cancelResponseComponent')
    .addEventListener('click', function () {
      responseOverlayE.classList.add('show');
    });

  document
    .querySelector('#saveResponseComponent')
    .addEventListener('click', async function () {
      if (this.classList.contains('disabled')) return;
      ...
      var requires =
        responseComponentSelect[
          responseComponentSelect.selectedIndex
        ].getAttribute('data-requires') || '';
      ...
      clickedAddComponentE.parentElement.parentElement.insertBefore(
        e,
        clickedAddComponentE.parentElement
      );
      renderComponents();
  ...
  document
    ...
    .addEventListener('click', async function () {
      ...
      var cardE = document.querySelector(
        '#responses > .card[data-id="' + editedResponseID + '"]'
      );
  ...
  responseComponentSelect.addEventListener('change', function () {
    var requires =
      this.options[this.selectedIndex].getAttribute('data-requires') || '';
    requires = requires.split('.');
    if (requires.includes('spotifyAccount')) {
  ...
  document
    .querySelector('#showNewResponseOverlayBTN')
    .addEventListener('click', function () {
  ...
  document
    ...
    .addEventListener('click', async function () {
      ...
      try {
        var resp = await fetch('/responses/verify?name=' + requestedName, {
          method: 'post',
        });
      } catch (e) {
  ...
  async function getComponents() {
    ...
    var data = await resp.text();
    responseComponentsE.innerHTML =
      data + '<div class="component blankComponent"></div>';
    renderComponents();
  }
  ...
  function flowStatesListeners() {
    ...
    for (let i = 0; i < es.length; i++) {
      if (es[i].getAttribute('data-handled') !== '1') {
        ...
        es[i].addEventListener('change', function () {
          stateConditionChanged(this);
        });
      }
    }
  ...
  document
    .querySelector('#showNewFlowOverlayBTN')
    .addEventListener('click', function () {
  ...
  document
    ...
    .addEventListener('click', async function () {
      ...
      try {
        var resp = await fetch('/flows/verify?name=' + requestedName, {
          method: 'post',
        });
      } catch (e) {
  ...
  document
    ...
    .addEventListener('click', async function () {
      ...
      var cardE = document.querySelector(
        '#flows > .card[data-id="' + editedFlowID + '"]'
      );
